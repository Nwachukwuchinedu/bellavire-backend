...
export const getAllLeases = async (req, res) => {
  try {
    ...
    const transformedLeases = leases.map((lease) => {
      ...
      leaseObj.tenantInfo =
        lease.tenantId && typeof lease.tenantId === "object"
          ? {
              id: lease.tenantId._id,
              name: `${lease.tenantId.firstName || ""} ${
...
export const getLeaseById = async (req, res) => {
  try {
    ...
    const lease = await Lease.findOne({
      ...
      .populate(
        "tenantId",
        "firstName lastName email phoneNumber address country city employmentStatus monthlyIncome"
      )
      .populate(
        "propertyId",
        "propertyName address monthlyRent depositAmount propertyType bedrooms bathrooms"
      )
    ...
    if (lease.tenantId && lease.propertyId) {
      try {
        paymentHistory = await TenantPayment.find({
          ...
          .select(
            "description amount dueDate transactionId status paymentMethod receipt"
          )
          .sort({ createdAt: -1 });
      } catch (paymentError) {
    ...
    leaseObj.propertyDetail =
      lease.propertyId && typeof lease.propertyId === "object"
        ? {
    ...
    leaseObj.landlordDetail =
      lease.landlordId && typeof lease.landlordId === "object"
        ? {
  ...
  try {
    ...
    try {
      ...
      if (tenant) {
        ...
        // Rental history created successfully
      }
    } catch (rentalHistoryError) {
      console.error(
        "Error creating rental history for lease terminated by landlord:",
        rentalHistoryError
      );
